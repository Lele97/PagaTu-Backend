...
public class TokenCleanupBatchJob {
    ...
    public void cleanupVerificationExpiredTokens() {

        log.debug("Starting token verification cleanup batch job at {}", LocalDateTime.now());
        ...
        try {
            ...
            initialStats = tokenCleanupMonitoringService.getVerificationTokenStatistics();
            ...
            finalStats = tokenCleanupMonitoringService.getVerificationTokenStatistics();
    ...
    public void cleanupExpiredTokens() {
        ...
        log.info("Starting token reset password cleanup batch job at {}", LocalDateTime.now());
     ...
     */
    @Retryable(maxAttempts = 3, backoff = @Backoff(delay = 1000, multiplier = 2))
    protected int performBatchTokenVerificationCleanup() {
        try {
            LocalDateTime currentTime = LocalDateTime.now();
            log.debug("Executing batch update for tokens expired before {}", currentTime);
            ...
            updatedCount = tokenForVerificationRepository.updateExpiredTokensStatus(
                    TokenStatus.EXPIRED,
                    currentTime,
                    TokenStatus.ACTIVE
            );
     ...
     */
    @Retryable(backoff = @Backoff(delay = 1000, multiplier = 2))
    protected TokenProcessingResult processTokensVerificationIndividually() {
        try {
            LocalDateTime currentTime = LocalDateTime.now();
            log.debug("Retrieving active tokens for individual processing at {}", currentTime);

            List<EmailVerificationToken> activeTokens = tokenForVerificationRepository.findAllByTokenStatus(TokenStatus.ACTIVE);
            log.info("Found {} active tokens to process", activeTokens.size());

            int updatedCount = 0;

            for (EmailVerificationToken token : activeTokens) {
                if (processSingleVerficationToken(token, currentTime)) {
                    updatedCount++;
                }
            }
     ...
     * @return true if the token was updated to expired status, false otherwise
     */
    private boolean processSingleVerficationToken(EmailVerificationToken token, LocalDateTime currentTime) {
        try {
            if (isTokenVerificationExpired(token, currentTime)) {
                updateTokenVerificationToExpired(token);
                log.debug("Updated token ID {} to EXPIRED status (expired at: {})",
                        token.getId(), token.getExpiredDate());
                return true;
            } else {
     ...
     * Checks if a token has expired by comparing its expiration date with current time.
     */
    private boolean isTokenVerificationExpired(EmailVerificationToken token, LocalDateTime currentTime) {
        return token.getExpiredDate() != null && token.getExpiredDate().isBefore(currentTime);
    }
     ...
     */
    @Retryable(maxAttempts = 3, backoff = @Backoff(delay = 500, multiplier = 1.5))
    protected void updateTokenVerificationToExpired(EmailVerificationToken token) {
        try {

            token.setTokenStatus(TokenStatus.EXPIRED);

            tokenForVerificationRepository.save(token);
